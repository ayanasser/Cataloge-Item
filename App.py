...
@app.route('/JSON')
@app.route('/cataloge/JSON/')
def Cataloge_JSON():
    Cat = session.query(Category).all()
    return jsonify(Cat=[cat.serialize for cat in Cat])

https://bit.ly/2Dsywz8?fbclid=IwAR35XHSRHzBvrk65IGVmjK-0VpZ_KG1oGE41lmP4WfkpeVPVWDjyMLldI5k
...
def show_item(category_name, item_name):
    ...
    return render_template("item.html", item=items,
                           category=category.name, creator=creator)
...
def delete_item(category_name, item_name):
    ...
    if request.method == 'POST':
        ...
        return render_template("deleteitem.html",
                               category=category.name, item=item.name)
